...
from transforms3d.euler import euler2mat, quat2euler, euler2quat, mat2euler
...
def carla_transform_to_numpy_matrix(carla_transform):
    t = carla_location_to_numpy_vector(carla_transform.location)
    r = carla_rotation_to_numpy_rotation_matrix(carla_transform.rotation)
    tf = numpy.concatenate((r, numpy.expand_dims(t, axis=0).T), axis=1)
    tf = numpy.concatenate((tf, numpy.array([[0, 0, 0, 1]])), axis=0)
    return tf

def carla_rotation_to_numpy_matrix(carla_rotation):
    r = carla_rotation_to_numpy_rotation_matrix(carla_rotation)
    tf = numpy.concatenate((r, numpy.array([[0, 0, 0, 1]]).T), axis=1)
    tf = numpy.concatenate((tf, numpy.array([[0, 0, 0, 1]])), axis=0)
    return tf
...
def numpy_vec_to_carla_location(vec):
    return carla.Location(vec[0], -vec[1], vec[2])
...
def numpy_matrix_to_carla_transform(mat):
    loc = numpy_vec_to_carla_location(mat[:3, 3])
    roll, pitch, yaw = mat2euler(mat[:3, :3])
    rot = RPY_to_carla_rotation(roll, pitch, yaw)
